...
public class ListaLaboratorioDAO
{
  ...
  "FROM [Isolamento_Versao_2].[dbo].[Laboratorio] WHERE Prontuario is not null";
    ...
    public static List<Laboratorio> ListarExamesLaboratorioPorPaciente(string _cod_laboratorio)
    {
        var lista = new List<Laboratorio>();
      ...
      ",[Resultado]" +
      ",[Prontuario]" +
  "FROM [Isolamento_Versao_2].[dbo].[Laboratorio] where prontuario =" + _cod_laboratorio;
            try
            {
                cnn.Open();
                SqlDataReader dr1 = cmm.ExecuteReader();
